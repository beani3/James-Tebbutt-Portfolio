...
Cube::Cube(unsigned int diffuseTexture, unsigned int specularTexture, unsigned int normalTexture ,float shine) : 
...
void Cube::makeVAO() {
	...
	glNamedBufferStorage(EBO, sizeof(unsigned int) * cubeIndicies.size(), cubeIndicies.data(), GL_DYNAMIC_STORAGE_BIT);
...
void Cube::setCubeMatVals(Shader* shader) {
	shader->use();
