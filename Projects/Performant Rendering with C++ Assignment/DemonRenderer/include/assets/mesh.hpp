...
using LOD_data = std::array<std::pair<uint32_t, uint32_t>, 3>;
...
struct Mesh
{
	...
	std::vector<uint32_t> allIndices; //!< All indices for LODs
	...
	std::array<std::pair<uint32_t, uint32_t>, 3> m_LOD_data;
	LOD_data getLODData() { return m_LOD_data; }
	std::vector<uint32_t> m_indicesLOD1;
	std::vector<uint32_t> m_indicesLOD2;
	size_t m_LODidx{ 0 };
	bool m_genLODs{ false };
};
...
{
public:
	Model(std::filesystem::path path, uint32_t options = positions | normals | uvs , bool genLODs = false); //!< Constructor with filepath and options
...
private:
	void processNode(aiNode* node, const aiScene* scene, bool genLODs); //!< Process a node within a heirachical model.
	void processMesh(aiMesh* mesh, const aiScene* scene, const glm::mat4& relativeTransform, bool genLODs); //!< Process a mesh from a node within a heirachical model.
