...
public:
	...
	bool uploadBuffer(size_t bindingPoint, const std::vector<unsigned char>& data) const;
private:
...
inline bool UBOManager::uploadBuffer(size_t bindingPoint, const std::vector<unsigned char>& data) const
{
	auto UBOptrPair = m_UBOsByBindingPoint.find(bindingPoint);
	if (UBOptrPair != m_UBOsByBindingPoint.end())

		return UBOptrPair->second->uploadBuffer(data);
