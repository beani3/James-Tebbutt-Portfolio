...
private:
	...
	size_t m_LODidx{ 0 };
	inline static uint32_t s_currShaderID{ std::numeric_limits<uint32_t>::max() }; //!< Static variable to track which material is currently bound, used to avoid redundant binds
	std::array<std::pair<uint32_t, uint32_t>, 3> m_LOD_data; //!< Stores vertex data with an indices offset for where amd how much data should be read
