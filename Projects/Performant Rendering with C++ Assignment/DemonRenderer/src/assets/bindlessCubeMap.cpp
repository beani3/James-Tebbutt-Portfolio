...
BindlessCubeMap::BindlessCubeMap(const std::array<const char*, 6>& filepaths, bool isHDR, bool flipUvs)
{
    ...
    if (isHDR) {
        ...
        if (channels == 3) {
            ...
            spdlog::error("BindlessCubeMap load error: {} channels are not supported (HDR).", channels);
        ...
        if (channels == 3) {
            ...
            spdlog::error("BindlessCubeMap load error: {} channels are not supported (non-HDR).", channels);
            stbi_image_free(dataf);
            return;
        }
    ...
    if (m_handle == 0) spdlog::error("Could not generate GPU resident handle for cubemap: {}", filepaths[0]);
    glMakeTextureHandleResidentARB(m_handle);
}

BindlessCubeMap::~BindlessCubeMap()
{
