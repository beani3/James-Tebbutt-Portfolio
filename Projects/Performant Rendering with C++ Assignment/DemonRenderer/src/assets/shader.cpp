...
/**
 * @brief Construct and compile a Shader based on the provided description.
 * @param desc ShaderDescription holding file paths and type.
 *
 * On success the program is linked and uniform/UBO information is cached.
 */
...
/**
 * @brief Compile a standard vertex/fragment program and link into program.
 * @param desc ShaderDescription with vertex and fragment source paths.
 */
...
/**
 * @brief Compile vertex/geometry/fragment program.
 */
void Shader::compileWithGeometry(const ShaderDescription& desc)
{
	...
	if (vertexCompiled && geoCompiled && fragmentCompiled)
	{
		...
		glDetachShader(m_ID, fragmentShader);
		/** delete source, dont need these anymore */
		glDeleteShader(vertexShader);
		glDeleteShader(geoShader);
		glDeleteShader(fragmentShader);
...
/**
 * @brief Compile with tessellation (control/eval) and fragment.
 */
...
/**
 * @brief Compile with tessellation + geometry + fragment.
 */
...
/**
 * @brief Compile a compute shader program.
 */
...
/**
 * @brief Read and compile a single GLSL shader unit from file.
 * @param shaderUnit Out parameter that will receive the created shader unit id.
 * @param sourcePath Path to the GLSL source file.
 * @param shaderType GL shader type enum (e.g. GL_VERTEX_SHADER).
...
/**
 * @name Uniform dispatch helpers
 * Helper overloads that translate C++ types to appropriate glUniform* calls.
 */
/** @{ */
